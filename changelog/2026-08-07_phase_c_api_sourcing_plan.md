...
**历史数据**：本方案**不处理历史数据**（用户定稿）。已入库旧接口 url 可能带域名、与 Excel 生成 url 形态不一致，查库阶段无法对齐 → 由 **D2 兜底**（告警 + 该用例不生成）。历史接口如需复用，须在方案上线后**重新导入**（`commit_api_docs` 走新规范化逻辑）；不做迁移、不做查询 fallback 兼容。
...
### Step 3 — SQLite 查询层
- 新增 `database/operations/api_ops.py`：`ApiOps.get_by_url(session, method, url) -> dict | None`（`doc_type='api'`，规范化比对，**无原始值 fallback**——历史 url 查不到走 D2），返回 `{api_name, api_url, api_method, api_headers, api_parameters, api_returns, api_annotations}` 反序列化后的完整 dict。
...
3. 历史数据不迁移、不做兼容 fallback；历史接口复用需重新导入（D1 决策）
