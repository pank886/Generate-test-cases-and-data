...
async def upload_file(file: UploadFile = File(...)):
    """上传文件（PDF/MD/Axure .zip）-> 按类型分目录存储 -> 存入向量库"""
    ...
    try:
        if ext == ".zip":
            ...
            result = process_axure_zip(file_path)
            ...
            result = process_product_doc(file_path)
            count = result.get("chunks", 0)
            source = "Word 文档"
            module_name = result.get("module_name")
            doc_id = result.get("doc_id")
        ...
        if count == 0:
            return JSONResponse(
                status_code=400,
                content={"success": False, "message": "文件解析后无内容，请检查文件是否有效。"},
            )
        _vector_ready = True
        ...
        print(f"✅ {source} 处理完成：{count} 个文本块")

        resp = {"success": True, "message": f"已处理 {count} 个文本块", "file": file_info}
        if module_name:
        ...
        return JSONResponse(status_code=400, content={"success": False, "message": "上传文件不存在"})
...
async def update_module(data: dict):
    """审核确认/修改模块关联关系"""
    try:
        ...
        db = DualChromaDB()
        # 更新 metadata.module（通过 doc_id 删除重写实现）
        db.product_store.delete(where={"doc_id": doc_id})
        print(f"   [Audit] doc_id={doc_id} 更新为 module={module_name}, related={related_modules}")
        # 注意：实际更新需要重新上传，这里仅记录日志
        # Phase A 全量流程上线后这里会做成真正的 metadata update
