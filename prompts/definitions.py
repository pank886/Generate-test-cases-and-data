...
class PromptFactory:
    ...
    def generate_excel_plan_node(self) -> ChatPromptTemplate:
        ...
        ("system",
         ...
         "4. **Allure 层级映射**（严格按照三级结构）：\n"
         "   - `allure_epic` = **项目名称**（模块的上级目录/根目录），如 `园区基线`、`集团平台`\n"
         "   - `allure_feature` = **模块名称**，如 `健身房管理`、`门禁签到`\n"
         "   - `allure_story` = **子模块名称或场景名称**，如 `健身房添加`、`人员签到`\n"
         "   - `case_name`(作为 Allure title) = **测试步骤说明**，如 `添加健身房-添加全部必填项-提交成功-查询健身房`\n"
         "   - `fixture_level`：逗号分隔，如 `smoke,danyuan`\n"
         "   示例：epic=`园区基线`，feature=`健身房管理`，story=`健身房添加`，case_name=`test_add_gym_all_required_001`\n"
