...
class PromptFactory:
  ...
  "required": ["data"]
    ...
    def generate_excel_plan_node(self) -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate.from_messages([
        ("system",
         "你是高级测试设计专家。根据【接口定义】和【用户意图】，生成 Excel 测试计划（每行一条用例），严格按以下规则输出 JSON。\n\n"
         "### 设计规则（优先级从高到低）\n"
         "1. **模块划分**：`module_name` 格式 `TestXxx_xxx`，按业务模块（如临停、包月）分组。\n"
         "2. **用例顺序**：同一模块内按业务流程依次排列（如入场→出场），后序依赖前序结果。\n"
         "3. **命名规范**：`case_name` 为 `test_xxx` 格式，`project_name` 取自用户意图（如 VehicleAccess）。\n"
         "4. **标签填写**：\n"
         "   - `fixture_level`：逗号分隔，如 `smoke,danyuan`（冒烟/单元/集成）。\n"
         "   - Allure 标签（`allure_epic`/`feature`/`story`）按层级描述业务。\n"
         "5. **步骤与数据**：`steps` 简明描述接口调用顺序，`test_data_yaml` 填入对应 YAML 文件名（如 `car_in.yaml`）。\n"
         "   - `allure_title` 填写用例标题，`precondition` 填写前置条件（无则填「无」）。\n"
         "6. **启用字段**：`enabled` 统一填 `Y`。\n"
         "7. **完整性**：所有必填字段不得为空（见 ExcelRow 定义）。\n\n"
         "### 输出格式\n"
         "必须输出 JSON 对象，包含以下字段：\n"
         "- `rows`：ExcelRow 对象数组（每条用例一个元素）\n"
         "- `file_name`：文件名，固定为 `test_plan.xlsx`\n"
         "示例：{{\"rows\": [{{\"project_name\": \"...\", ...}}], \"file_name\": \"test_plan.xlsx\"}}\n"
    ...
    def generate_py_class_node(self) -> ChatPromptTemplate:
        ...
        ("system",
         ...
         "    @allure.title('<用例标题>')\n"
         "    @pytest.mark.order(n)\n"
         "    @pytest.mark.parametrize('params', get_testcase_yaml('./testcase/<项目名>/<yaml文件名>'))\n"
