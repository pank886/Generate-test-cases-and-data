...
class PromptFactory:
    ...
    def generate_excel_plan_node(self) -> ChatPromptTemplate:
        ...
        ("system",
         ...
         "  错误: \"1.创建成功\"（缺关键词） / \"1.(eq)创建成功\"（括号类型错） / \"1.[ eq ]创建成功\"（含空格）\n\n"
         "### 规则\n"
    ...
    def analyze_test_points_raw(self) -> ChatPromptTemplate:
        ...
        return ChatPromptTemplate.from_messages([
            ("system",
                ...
                ## 测试场景分析
                按模块逐一分析：模块功能概述、涉及接口、测试策略（正向/反向/边界），以及该模块与其他模块的数据依赖关系。
                ...
                ## 测试用例
                所有用例必须严格遵循以下缩进与换行格式（{{}}内为变量占位符）：
                - TC-{{全局编号}}: {{模块名}}-{{功能点}}-{{场景类型}}
                    子模块: {{从文档提取的二级模块名}}
                    前置: {{PRE编号}} 或 无
                    步骤: 1.{{操作步骤1}}
                          2.{{操作步骤2}}
                ...
                ### ⚠️ 强制规则（违反将导致用例被丢弃）
                - **共享前置段落绝对不能为空！** 每个测试计划至少有一条共享前置。常见的共享前置：创建测试数据（测试电表、测试住户、测试企业）、创建配置（结算配置、计费方案、公摊配置）。至少输出一条。
                - PRE-xxx 从 PRE-001 开始，全局唯一，递增。TC-xxx 同理
                - 前置字段直接引用 PRE 编号（如 PRE-001, PRE-002），禁止写「执行共享前置X」
                - 前置引用只能是 PRE-xxx，禁止写 TC-xxx 或其他格式
                - **严禁步骤与预期不对齐！** 步骤有 N 条 → 预期必须有 N 条。若步骤只有 1 条，预期绝对不能写 3 条——必须拆分步骤或合并预期。每个用例写完立即自查
    ...
    def generate_dependency_map(self) -> str:
        ...
        返回系统消息文本字符串（含 {factory_methods} 占位符）。
        """
